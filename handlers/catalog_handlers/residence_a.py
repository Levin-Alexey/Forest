...
async def residence_a_handler(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "<b>Благодарю за интерес к «Pisateli Forest». Вы на пороге выбора резиденции, где премиальный комфорт уже заложен в основу каждой детали.</b>\n\n"
        "Мы понимаем, что покупка дома - это только начало пути. Чтобы избавить вас от сложностей ремонта и поиска подрядчиков, мы разработали стандарт «Ready for life».\n"
        ...
        "• <b>Инженерный фундамент комфорта:</b> Система многоступенчатой очистки воздуха, теплые полы, скрытое управление климатом и подогрев всех входных групп (крыльцо, парковочные места).\n"
        ...
        "Перечислить всё невозможно, да и вряд ли текст передаст ощущение от тактильных материалов, аромата хвои в панорамных окнах или того, как инженерные системы работают в идеальном тандеме. <b>Это тот случай, когда лучше один раз увидеть всё своими глазами, чем изучать списки характеристик.</b> Каждая деталь в «Pisateli Forest» создана, чтобы удивлять при личной встрече.\n"
        "---\n"
        "🏠 <b>Индивидуальный подход «Под тапочки»:</b>\n"
        "Если вы цените максимальную готовность, мы можем пойти дальше. Мы укомплектуем резиденцию мебелью и аксессуарами — либо строго по авторскому дизайн-проекту, либо адаптируем интерьер под ваши личные пожелания, чтобы ваш переезд стал легким и приятным.\n"
